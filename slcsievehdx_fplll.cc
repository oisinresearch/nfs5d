	...
	int mbb, int bb)
{
	...
	while (p < Bmax) {
		...
		for (int j = 0; j < ni; j++) {
			...
			// reduce L using fplll
			lll_reduction(L, LLL_DEF_DELTA, LLL_DEF_ETA, LM_FAST, FT_DOUBLE, 0, LLL_DEFAULT);
			
			// convert column-major L to flattened row-major L2
			for (int k = 0; k < d; k++)
				for (int l = 0; l < d; l++)
					L2[k*d + l] = L(l, k).get_si();

			// enumerate all vectors up to radius R in L2, up to a max of 1000 vectors
			int nn = enumeratehd(d, d, L2, M, m, logp, p, R, 1000, mbb, bb);
			nntotal += nn;
		}
